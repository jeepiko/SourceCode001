...
@Component({
  selector: 'infostan',
  templateUrl: './infostan.html',
  styleUrls: ['./infostan.css']
  , providers: [DatePipe]
})
export class InfoStan {
  ...
  read_productsforpdf(pdfjson) {
    ...
    for (var i = 0; i < pdfjson.length; i++) {
      ...
      this.content.push({
        text: 'xxxxxx   Osijek '
          + '                                                                                                 '
          + 'xxxxxxxxx  Osijek', margin: [120, 0, -5, 0], fontSize: 8
      });
  ...
  read_productsforpdfplin(pdfjson) {
    ...
    for (var i = 0; i < pdfjson.length; i++) {
      ...
      this.content.push({ text: pdfjson[i].rUpraviteljNaziv + ' ' + pdfjson[i].rUpraviteljUlica + ' , ' + pdfjson[i].rUpraviteljMjesto, margin: [0, 0, 0, 0], fontSize: 10, alignment: 'center' });
      this.content.push({ text: 'OIB:   ' + pdfjson[i].rUpraviteljOIB, margin: [0, 0, 0, 0], fontSize: 10, alignment: 'center' });
      this.content.push({ text: 'e-mail: ' + pdfjson[i].rUpraviteljMail, margin: [0, 0, 0, 15], fontSize: 10, alignment: 'center' });
      this.content.push({ text: pdfjson[i].rVlasnikNaziv, margin: [17, 0, 0, 0], fontSize: 12, bold: true });
      this.content.push({ text: pdfjson[i].rVlasnikUlica, margin: [17, 0, 0, 0], fontSize: 12, bold: true });
      this.content.push({ text: pdfjson[i].rVlasnikMjesto, margin: [17, 0, 0, 0], fontSize: 12, bold: true });
      this.content.push({ text: 'Obračun broj: ' + pdfjson[i].rBrojObracuna, margin: [0, 0, 0, 0], fontSize: 10, alignment: 'right' }, );
      ...
      this.content.push({
        ...
        table: {
          ...
          body: [
            ['Datum obračuna: ' + pdfjson[i].rDatumObracuna, 'Poziv na broj za ovaj račun je :' + pdfjson[i].rPozivNaBroj],
            ['Datum dospijeća: ' + pdfjson[i].rDatumDospijeca, 'Obračun sastavila : Jasmina B.  '],
            ['Datum isporuke:  ' + pdfjson[i].rDatumIsporuke, 'Obračun raspodjele plina sastavio : TEH-PROJEKT OPREMA d.o.o. Rijeka'],
          ]
        }
        ,
        layout: {
      ...
      this.content.push({ text: '', margin: [0, 0, 0, 5], fontSize: 7 });
      var rows = [];
      for (var y = 0; y < pdfjson[i].naknade.length; y++) {
        rows.push([pdfjson[i].naknade[y].opis + ' ' + pdfjson[i].naknade[y].iznos + ' kn          ']);
      }
      ...
      var rowss = [];
      for (var y = 0; y < pdfjson[i].plin.length; y++) {
        rowss.push([pdfjson[i].plin[y].iznos + ' kn          ']);
      }
      this.content.push({ text: 'Zajednički troškovi energetskog postrojenja i utrošak plina za mjesec : ' + pdfjson[i].plin[0].opis, margin: [17, 0, 0, 3], fontSize: 10, bold: true, alignment: 'center' });
      this.content.push({
        style: 'tableExample', margin: [160, 0, 160, 3], fontSize: 8, alignment: 'right',
        table: {
          widths: ['*'],
      ...
      for (var yy = 0; yy < 4 - pdfjson[i].naknade.length; yy++) {
        this.content.push({ text: ' ', margin: [0, 0, 0, 5], fontSize: 8 });
      }


      this.content.push({ text: 'Zajednička pričuva stambene zgrade obračunata je prema Međuvlasničkom ugovoru i Ugovoru o upravljanju ', margin: [10, 0, 0, 0], fontSize: 8 });

      this.content.push({ text: 'br. ' + pdfjson[i].rUgovorBroj + ' od  ' + pdfjson[i].rUgovorDatum + ' godine. ', margin: [10, 0, 0, 7], fontSize: 8 });
      this.content.push({ text: 'NAPOMENA: PDV nije obračunat - doprinos za zajedničku pričuvu  nije predmet oporezivanja  temeljem  ', margin: [10, 0, 0, 0], fontSize: 8 });
      this.content.push({ text: 'članka 4 Zakona o PDV-u. Zgrada nije u sustavu PDV , jer nije fizička ni pravna osoba.  ', margin: [10, 0, 0, 0], fontSize: 8 });
      this.content.push({ text: 'Eventualne reklamacije stanar je dužan izvršiti bez odlaganja, a najkasnije u roku od 8 dana od primitka obračuna. ', margin: [10, 0, 0, 5], fontSize: 8 });
      this.content.push({ text: 'Kod prekoračenja roka plaćanja izračunavamo zateznu kamatu. ', margin: [10, 0, 0, 0], fontSize: 8 });
      this.content.push({ text: 'Obračun je punovaljan bez žiga i potpisa jer je tiskan elektroničkim računalom. ', margin: [10, 0, 0, 1], fontSize: 8 });
      this.content.push({ text: 'Raspodjela zajedničkih troškova energetskog postrojenja prema odluci stanara od 16.10.2010. i utrošak plina za mjesec : ' + pdfjson[i].plin[0].opis + '.', margin: [10, 0, 0, 8], fontSize: 8 });
      this.content.push({ text: 'Stambena zgrada:', margin: [0, 0, 0, 0], fontSize: 10, alignment: 'right' });
      this.content.push({ text: pdfjson[i].rZgradaUlica1 + ' ' + pdfjson[i].rZgradaMjesto1, margin: [0, 0, 0, 0], fontSize: 10, alignment: 'right' });
      this.content.push({ text: 'Upravitelj: ' + pdfjson[i].rUpraviteljNaziv1, margin: [0, 0, 0, 0], fontSize: 10, alignment: 'right' });

      this.content.push({
        columns: [
          { text: 'UPOZORENJE:', margin: [10, 0, 0, 0], fontSize: 8 },
          { text: 'Direktor  : ' + pdfjson[i].rUpraviteljDirektor, margin: [0, 0, 0, 4], fontSize: 10, alignment: 'right' }
        ]
      });

      this.content.push({ text: 'U slučaju neispunjavanja dospjele novčane obveze, vjerovnik može zatražiti određivanje ovrhe na temelju vjerodostojne isprave.', margin: [10, 0, 0, 15], fontSize: 8 });
      var iznosnaloga = pdfjson[i].vIznos
      this.content.push({
        columns: [
          { text: pdfjson[i].vValuta, margin: [182, 0, 0, 0], fontSize: 8 },
