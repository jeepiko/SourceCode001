...
import { InfoStanSlog } from './infostanslog';
...
export class HomePage {
  ...
  selectedRowwkor: Number;
  setClickedRowwkor: Function;
  ...
  productskorisnik: WKorSlog[];
  ima_korisnik: boolean = false;

  productsmajstor: WKorSlog[];
  ima_majstor: boolean = false;
  productspredstavnik: WKorSlog[];
  ima_predstavnik: boolean = false;

  productsprikazi: WKorSlog[];
  productsprikazioznaka: string;


  prikazi_btnstan: boolean = false;
  prikazi_btnmajstor: boolean = false;
  prikazi_btnpredstavnik: boolean = false;
  position = 'before';
  mybuttonbgcolorKorisnik: string;
  mybuttonbgcolorMajstor: string;
  mybuttonbgcolorPredstavnik: string;

  mybuttoncolorKorisnik: string;
  mybuttoncolorMajstor: string;
  mybuttoncolorPredstavnik: string;




  naslov: string;
  prikazistavkeinfo: boolean = false;
  prikazistavkepredstavnik: boolean = false;


  productswkorpregled: any[];
  ...
  textcardheader: string;

  izmjenaForm: boolean = false;
  izmjenaPotvrdi: boolean = false;
  izmjenaOK: boolean = false;
  izmjenaNEOK: boolean = false;
  ...
  barcodetextslog: InfoStanSlog;

  mojaboja: string

  constructor(private datePipe: DatePipe, private location: Location, public sifrdataservice: SifrDataService, private authenticationService: AuthenticationService, private formBuilder: FormBuilder, public router: Router, private http: Http, private dataService: DataService) {
  }
  ...
  ngOnInit() {
    this.izmjenaOK = false;

    this.setClickedRow = function (index) {
      this.selectedRow = index.BrojStana;
    }

    this.setClickedRow1 = function (index) {
      this.selectedRow1 = index.Racun;
    }



    this.setClickedRowwkor = function (index) {
      this.selectedRowwkor = index.Racun;
    }

    this.username = localStorage.getItem('user_uname');
    this.user_id = localStorage.getItem('user_id');
    this.useradrs = localStorage.getItem('user_fname');
    this.user_username = localStorage.getItem('user_username');



    this.prikazistavkeinfo = false;
    this.prikazistavkepredstavnik = false;

    this.userForm = this.formBuilder.group({

      'proizvod': ['', Validators.required],
      'jedmjere': ['', Validators.required],
    ...
    this.userForm1 = this.formBuilder.group({
      ...
      'proizvod1': ['', Validators.required],
      'jedmjere1': ['', Validators.required],
      'kolicina1': ['', Validators.required],
      'cijena1': ['', Validators.required],
      'iznos1': ['', Validators.required],
    });
    ...
    this.ima_korisnik = false;
    this.ima_predstavnik = false;
    this.ima_majstor = false;

    this.readMyAllWKorisnik('0');
  ...
  readMyAllWKorisnik(inda) {


    this.prikazistavkeinfo = false;
    this.prikazistavkepredstavnik = false;

    this.mybuttonbgcolorKorisnik = "white";
    this.mybuttonbgcolorMajstor = "#ADDCEE";
    this.mybuttonbgcolorPredstavnik = "#ADDCEE";

    this.mybuttoncolorKorisnik = "black";
    this.mybuttoncolorMajstor = "#FFFFFF";
    this.mybuttoncolorPredstavnik = "#FFFFFF";



    this.dataService.serviceAllWKorisnikPhp("A", this.user_id).subscribe(
      data => {

        this.productskorisnik = data;
        if (data.length > 0) {
          if (inda === 'A' || inda === '0') {

            this.productsprikazi = data;
            this.productsprikazioznaka = "korisnik";
          }

          this.ima_korisnik = true;
          this.naslov = "Vlasnik stana :";

        }


        this.readMyAllWMajstor(inda);
  ...
  readMyAllWMajstor(inda) {

    this.mybuttonbgcolorKorisnik = "#ADDCEE";
    this.mybuttonbgcolorMajstor = "white";
    this.mybuttonbgcolorPredstavnik = "#ADDCEE";
    this.mybuttoncolorKorisnik = "#FFFFFF";
    this.mybuttoncolorMajstor = "black";
    this.mybuttoncolorPredstavnik = "#FFFFFF";
    this.dataService.serviceAllWKorisnikPhp("B", this.user_id).subscribe(

      data => {
        this.productsmajstor = data;

        if (data.length > 0) {
          if (inda === 'B' || inda === '0') {
            this.productsprikazi = data;
            this.productsprikazioznaka = "majstor";
          }

          this.ima_majstor = true;
          this.naslov = "Kooperant :";
        }

        this.readMyAllWPredstavnik(inda);
  ...
  readMyAllWPredstavnik(inda) {
    ...
    this.mybuttonbgcolorKorisnik = "#ADDCEE";
    this.mybuttonbgcolorMajstor = "#ADDCEE";
    this.mybuttonbgcolorPredstavnik = "white";
    this.mybuttoncolorKorisnik = "#FFFFFF";
    this.mybuttoncolorMajstor = "#FFFFFF";
    this.mybuttoncolorPredstavnik = "black";



    this.dataService.serviceAllWKorisnikPhp("C", this.user_id).subscribe(

      data => {
        this.productspredstavnik = data;


        if (data.length > 0) {
          if (inda === 'C' || inda === '0') {
            this.productsprikazi = data;
            this.productsprikazioznaka = "predstavnik";
          }

          this.ima_predstavnik = true;
          this.naslov = "Predstavnik stanara :";
        }

        this.popuni_prikazi(inda);
  ...
  popuni_prikazi(inda) {
    ...
    if (this.ima_korisnik && this.ima_majstor != true && this.ima_predstavnik != true) {


      if (this.productsprikazi.length == 1) {

        var product = this.productsprikazi[0];
      }
    }
    this.prikazi_btnstan = false;
    this.prikazi_btnmajstor = false;
    this.prikazi_btnpredstavnik = false;

    if (this.ima_korisnik && this.ima_majstor) {
      this.prikazi_btnstan = true;
      this.prikazi_btnmajstor = true;
      this.prikazi_btnpredstavnik = false;
    }
    if (this.ima_korisnik && this.ima_predstavnik) {
      this.prikazi_btnstan = true;
      this.prikazi_btnmajstor = false;
      this.prikazi_btnpredstavnik = true;
    }
    if (this.ima_korisnik && this.ima_majstor && this.ima_predstavnik) {
      this.prikazi_btnstan = true;
      this.prikazi_btnmajstor = true;
      this.prikazi_btnpredstavnik = true;
    }
  ...
  showInfoStan(product: WKorSlog) {

    this.podacistan = product;
    this.nazivstanje = "Dugovanje:";
    this.podacistanje = this.podacistan.Stanje.toString();
    if (Number(this.podacistan.Stanje) < 0) {
      this.nazivstanje = "Više uplaćeno :"
      var pom = Number(this.podacistan.Stanje) * -1;
      this.podacistanje = pom.toString();
    }


    this.selectedRow = product.BrojStana;
    this.prikazistavkeinfo = true;

    this.sifrdataservice.serviceVariableWKorSlog = product;
    this.router.navigate(['infostan']);
  }
  showInfoMajstor(product: WKorSlog) {
    this.podacistan = product;
    this.sifrdataservice.serviceVariableWKorSlog = product;
    this.router.navigate(['infomajstor']);
  }
  showInfoPredstavnik(product: WKorSlog, vrsta: string) {
    this.podacistan = product;

    this.prikazistavkeinfo = false;
    this.prikazistavkepredstavnik = false;
    if (vrsta == '1') {
      this.textcardheader = "Pregled računa za ulaz"
    }
    if (vrsta == '2') {
      this.textcardheader = "Pregled stanova za ulaz"
    }
    if (vrsta == '3') {
      this.textcardheader = "Pregled kooperanata za ulaz"
    }
    if (vrsta == '4') {
      this.textcardheader = "Pregled obračuna plaće predstavnika"
    }
    product.Vrsta = vrsta;
    this.sifrdataservice.serviceVariableWKorSlog = product;
    this.readMyAllWpredstavnikpregled();
    this.router.navigate(['infopredstavnik']);
  }
  ...
  readMyAllWkorpregledInfo() {


    var stan = this.sifrdataservice.serviceVariableWKorSlog.BrojStana;

    this.dataService.serviceWkorpregledPhp(stan).subscribe(
      data => {
        setTimeout(() => { this.loaderInda = true; }, 1000);
  ...
  readMyAllWpredstavnikpregled() {
    ...
    this.dataService.serviceWpredstavnikpregledPhp(ulaz, vrsta, "", "", "").subscribe(
      data => {
  ...
  ispisiPregObrStan(productwkorpregled: InfoStanSlog) {
    this.selectedRow1 = Number(productwkorpregled.Racun);
    this.selectedRowwkor = Number(+productwkorpregled.Racun);
    this.dataListCtrl = [];

    const slogCtrl = new SlogCtrl(productwkorpregled.Racun);
    this.dataListCtrl.push(slogCtrl);

    if (productwkorpregled.Plin === "NE") {
      ...
      this.dataService.ispisiPregObrJsonPlin(this.dataListCtrl).subscribe(
        data => {
          this.productsforpdf = data;
          if (data.length) {

            // -- >>      this.read_productsforpdfplin(data);
          }
        },
        err => {
          //console.log(err);
  ...
  ////////////////////////////////////////////////////////////////
  ispisiPregObrBarCode(productwkorpregled: InfoStanSlog) {

    this.selectedRow1 = Number(productwkorpregled.Racun);

    this.barcodetextslog = productwkorpregled;
    ...
    if (productwkorpregled.Plin === "NE") {
      this.dataService.ispisiPregObrJson(this.dataListCtrl).subscribe(
        data => {
          this.productsforpdf = data;
          this.read_productsforbarcode(data);
        },
        err => {
          //console.log(err);
        },
  ...
  read_productsforpdf(pdfjson) {
    ...
    for (var i = 0; i < pdfjson.length; i++) {
      ...
      this.content.push({ text: pdfjson[i].rUpraviteljNaziv + ' ' + pdfjson[i].rUpraviteljUlica + ' , ' + pdfjson[i].rUpraviteljMjesto, margin: [0, 0, 0, 0], fontSize: 10, alignment: 'center' });
      ...
      this.content.push({ text: 'U slučaju neispunjavanja dospjele novčane obveze, vjerovnik može zatražiti određivanje ovrhe na temelju vjerodostojne isprave.', margin: [10, 0, 0, 20], fontSize: 8 });
      ...
      this.content.push({
        columns: [
          { text: pdfjson[i].vValuta, margin: [182, 0, 0, 0], fontSize: 8 },
          { text: this.padleft(iznosnaloga, 15, "*"), margin: [103, 0, 0, 0], fontSize: 8 },
          { text: pdfjson[i].vValuta + ' ' + this.padleft(iznosnaloga, 15, "*"), margin: [0, 0, 63, 0], fontSize: 8, alignment: 'right' }
        ]
      });

      this.content.push({ text: '', margin: [0, 0, 0, 8], fontSize: 7 });
      this.content.push({
        columns: [
      ...
      this.content.push({ text: '', margin: [0, 0, 0, 10], fontSize: 8 });
      this.content.push({
        columns: [
      ...
      this.content.push({ text: '', margin: [0, 0, 0, 13], fontSize: 8 });
      this.content.push({
        columns: [
      ...
      this.content.push({
        columns: [
          ...
          { text: pdfjson[i].vOpis1, margin: [-10, 0, -15, 0], fontSize: 6, alignment: 'right' }
        ]
      });
      ...
      this.content.push({
        columns: [
          { width: 390, text: pdfjson[i].vOpis2, margin: [200, 0, 0, 0], fontSize: 8 },
          { text: pdfjson[i].vOpis2, margin: [0, 0, -15, 0], fontSize: 6, alignment: 'right' }
        ]
      });
      this.content.push({
        columns: [
          { width: 390, text: pdfjson[i].vOpis3, margin: [200, 0, 0, 0], fontSize: 8 },
          { text: pdfjson[i].vOpis3, margin: [0, 0, -15, 0], fontSize: 6, alignment: 'right' }
        ]
      });

      this.content.push({ text: '', margin: [0, 0, 0, 8], fontSize: 8 });
      ...
      this.content.push({ text: '', margin: [0, 0, 0, 18], fontSize: 8 });
    ...
    var pdfname = "MaiorRD-" + dateFormat;
  ...
  read_productsforbarcode(pdfjson) {
    this.barcodeimg = "";
    this.content = [];

    for (var i = 0; i < pdfjson.length; i++) {
      this.barcodeimg = this.pdf417code(pdfjson[i]);
    }
  }
  ...
  pdf417code(pdfjsontext) {
    ...
    const imgsrc = generateBarcode(pdf417text, 2.3, 2);
  ...
  showIzmjenaForm() {
    this.izmjenaOK = false;
    this.izmjenaForm = true;
    this.newProduct.Korime = this.user_username;
    this.izmjenaPotvrdi = false;
  }

  cancelNewForm() {
    this.izmjenaOK = false;
    this.izmjenaForm = false;
    this.izmjenaPotvrdi = false;
    this.izmjenaNEOK = false;
    this.barcodeForm = false;
  }

  savePotvrdi() {
    this.izmjenaPotvrdi = true;
    this.izmjenaOK = false;
    this.izmjenaNEOK = false;
  }
  saveIzmjena(product) {
    ...
    //console.log ( "product.username",product.Korime) ;
    this.izmjenaOK = false;
    this.izmjenaNEOK = false;

    this.dataService.serviceWIzmjenaLozPhp(product.Korime, product.Lozinkas, product.Lozinkan).subscribe(

      data => {
        //    this.router.navigate([this.returnUrl]);
        this.izmjenajson = data;
        //console.log ( " ...this.izmjenajson[0].Odgovor "+this.izmjenajson[0].Odgovor) ; 
        if (this.izmjenajson[0].Odgovor === "UREDU") {
          this.izmjenaOK = true;
        } else {
          this.izmjenaNEOK = true;
          this.izmjenaPotvrdi = false;
        }
      },
      error => {
  ...
  setMyStyles(stanje) {
    //   console.log ( " ...setMyStyles()setMyStyles() "+stanje) ; 
