...
import { InfoMajstorSlog } from './infomajstorslog';
...
@Component({
  selector: 'infomajstor',
  templateUrl: './infomajstor.html',
  styleUrls: ['./infomajstor.css']
  , providers: [DatePipe]

})
export class InfoMajstor {
  ...
  newProductStar: any = {};
  private tt_id: number;
  private tt_name: string;
  private tt_description: string;
  ...
  onDateChanged_datum_racuna_od(event: IMyDateModel) {
    console.log(" ----- onDateChanged_datum_racuna_od ----> " + event.formatted);
    this.pom_datum_racuna_od = event.formatted;
  }
  onDateChanged_datum_racuna_do(event: IMyDateModel) {
    console.log(" ----- onDateChanged_datum_racuna_do ----> " + event.formatted);
  ...
  readMyAllWmajstorpregled() {


    var ulaz = this.sifrdataservice.serviceVariableWKorSlog.BrojUlaza;
    var majstor = this.user_id;

    this.dataService.serviceWmajstorpregledPhp(majstor, ulaz).subscribe(

      data => {

        this.productswmajstorpregled = data;
  ...
  ispisiPregObrStan(productwkorpregled: InfoMajstorSlog) {
    this.dataListCtrl = [];
  ...
  read_productsforpdf(pdfjson) {
    ...
    for (var i = 0; i < pdfjson.length; i++) {
      ...
      //  var pdfbarcode = this.pdf417code(pdfjson[i]) ;
      console.log("aaaaaa---for (var i = 0; i <  pdfjson.length; i++")
      ...
      this.content.push({
        text: 'xxxx   Osijek '
          + '                                                                                                 '
          + 'xxxx d.o.o   Osijek', margin: [120, 0, -5, 0], fontSize: 8
      });
